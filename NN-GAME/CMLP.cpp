							...
							)
{
	...
	for(i=0;i<ulNumberOfHiddenNodes;i++)
	{
		for(j=0;j<ulNumberOfInputs+1;j++)
		{
			ppdwih[i][j] += dStepSize * (((float)rand() / (float)RAND_MAX/2.0) - 1);
		}
	}
	...
	for(i=0;i<ulNumberOfOutputs;i++)
	{
		for(j=0;j<ulNumberOfHiddenNodes+1;j++)
		{
			ppdwho[i][j] += dStepSize * (((float)rand() / (float)RAND_MAX/2.0) - 1);
		}
	}
							...
							)
{
	...
	for(i=0;i<ulNumberOfHiddenNodes;i++)
	{
		...
		{
			dStimulus += ppdwih[i][j] * pdInputs[j-i];
		}
								...
								)
{
	...
	for(i=0;i<ulNumberOfPatterns;i++)
	{
		...
		for(j=0;j<ulNumberOfOutputs;j++)
		{
			...
			dError += 0.5 * pow(ppdTargets[i][j] - pdOutputs[j], 2);
...
int CMLP::Load(const char * const pFileName)
{
	...
	ifstream *pIn=new ifstream(pFileName,ios::_Nocreate);
