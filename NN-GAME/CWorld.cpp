...
extern double dErrorBias,dErrorVariance;

//Where example hit data will be saved to create the exemplar set with which the network 
//is trained
...
CWorld::CWorld(const unsigned long culNewTerrainResolution)
{
	...
	{
		pHitData = new std::ofstream(FileForExemplarData, std::ofstream::out);
...
void CWorld::Initialise(void)
{
	...
	//Initialise the parameters that control the random errors made by the AI tank. The bias
	...
	//each tank. The improves the appearance of the game since the tank graphics 
	...
	for(i=20;i<250;i++)
	{
		//The flat regions are identified measuring the variance of the landscape
		//height over stretches of 20 elements in the landscape height array. Low
		//variances implies flat lanscape.
		dMeanHeightInInterval=0.0;

		//First, calculate the mean height in the landscape in the interval 
...
CWorld::~CWorld()
{
	...
	//The height data should always exists at this time but check anyway
	if(plHeight!=NULL)
	{
		delete []plHeight;
	}
...
void CWorld::GenerateTerrain(void)
{
	...
	//Since we only have on point on the terrain, its height must be the
	//lowest and highest so far
	lLowest=plHeight[0];
	lHighest=plHeight[0];

	//p is a variable used to control the probability that the next element in 
	//the terrain array will be higher of lower than the current one. This approach
...
BOOL CWorld::TimeStep(void)
{
	...
	if(pProjectile)
	{
		//Check to see if the projectile has passed beyond the edge of the window
		...
		if(ppPlayer[1-nActivePlayer]->nTestForProjectileTankCollision(pProjectile))
		{
			...
			if(boGeneratingTrainingData)
			{
				//Record the relative positions of the tanks, the wind speed...
				*pHitData<<(-ppPlayer[nActivePlayer]->dxPosition+ppPlayer[1-nActivePlayer]->dxPosition);
				*pHitData<<"\t";
				*pHitData<<(-ppPlayer[nActivePlayer]->dyPosition+ppPlayer[1-nActivePlayer]->dyPosition);
				*pHitData<<"\t";
				*pHitData<<dWindSpeed;
				*pHitData<<"\t";
				//...and the inclination of the tank's barrel that scored the hit
				*pHitData<<ppPlayer[nActivePlayer]->dInclination;
				*pHitData<<"\n";
			}
		...
		//Calculate the x and t movement of the projectile in this time step
